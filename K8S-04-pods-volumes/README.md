...
- Create a `ondia-pv.yaml` file using the following content with the volume type of `hostPath` to build a `PersistentVolume` and explain fields.
...
- Create the PersistentVolume `ondia-pv-vol`.

```bash
kubectl apply -f ondia-pv.yaml
```
...
- Create a `ondia-pv-claim.yaml` file using the following content to create a `PersistentVolumeClaim` and explain fields.
...
- Create the PersistentVolumeClaim `ondia-pv-claim`.

```bash
kubectl apply -f ondia-pv-claim.yaml
```
...
- View information about the `PersistentVolumeClaim` and show that the `PersistentVolumeClaim` is bound to your PersistentVolume `ondia-pv-vol`.

```bash
kubectl get pvc ondia-pv-claim
```
...
- Create a `ondia-pod.yaml` file that uses your PersistentVolumeClaim as a volume using the following content.
...
spec:
  volumes:
    - name: ondia-pv-storage
      persistentVolumeClaim:
        claimName: ondia-pv-claim
  containers:
    - name: ondia-pv-container
      image: nginx
      ports:
        - containerPort: 80
          name: "http-server"
...
- Create the Pod `ondia-pod`.

```bash
kubectl apply -f ondia-pod.yaml
```

- Verify that the Pod is running.

```bash
kubectl get pod ondia-pod
```

- Open a shell to the container running in your Pod.

```bash
kubectl exec -it ondia-pod -- /bin/bash
```
...
- Expose the ondia-pod pod as a new Kubernetes service on master.

```bash
kubectl expose pod ondia-pod --port=80 --type=NodePort
```
...
- Check the browser (`http://<public-workerNode-ip>:<node-port>`) that ondia-pod is running.

- Delete the `Pod`, the `PersistentVolumeClaim` and the `PersistentVolume`.

```bash
kubectl delete pod ondia-pod
kubectl delete pvc ondia-pv-claim
kubectl delete pv ondia-pv-vol
```

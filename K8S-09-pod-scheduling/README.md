...
- Create yaml file named `ondia-deploy.yaml` and explain its fields.
...
```bash
kubectl apply -f ondia-deploy.yaml
```
...
```bash
kubectl delete -f ondia-deploy.yaml 
```
...
- Update the ondia-deploy.yaml as below.
...
- Now, we will update ondia-deploy.yaml as below. We just add `nodeSelector` field.
...
- We will update ondia-deploy.yaml as below. We will add an affinity field instead of nodeSelector.
...
- This time we will test `preferredDuringSchedulingIgnoredDuringExecution`. Update ondia-deploy.yaml as below. This time, we change `requiredDuringSchedulingIgnoredDuringExecution` with  `preferredDuringSchedulingIgnoredDuringExecution`.
...
- create yaml file and named `ondia-db.yaml`.
...
-  Create yaml file named `ondiashop-deploy.yaml`.
...
- List the pods and notice that the ondiashop pods are assigned to the same nod with ondia-db pod.
...
- This command places a taint on node kube-worker. The taint has key color, value blue, and taint effect NoSchedule. This means that no pod will be able to schedule onto kube-worker unless it has matching toleration.
...
- List the pods and notice that the pods are running on both the master node and the worker node.

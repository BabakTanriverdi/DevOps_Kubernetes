...
The purpose of this hands-on training is to give students the knowledge of  Autoscaling and Microservices
...
In this study, we will present an application that contains 2 unrelated sub-applications. 

- First one is a simple web&database application to hold the to-do-lists. This sub-application uses MongoDB to store to-do lists created through the web application. For the front-end web application layer, Node.JS is used. Thus, this sub-application has 2 microservices.
  
- The second one is used to demonstrate Horizontal Pod Autoscaler functionality based on the php-apache image.
...
2. And then deploy the `php-apache` app and highlight some important points.
3. The Autoscaling in Kubernetes will be demonstrated as the last step.

## Part 3 - Microservices

- The MongoDB application will use static volume provisioning with the help of a persistent volume (PV) and a persistent volume claim (PVC). 
        ...
        - image: clarusway/todo
          ...
          resources:
            ...
            requests:
              ...
              cpu: 80m		  
```
- Note that this web app is connected to the MongoDB host/service via the `DBHOST` environment variable. What does `db-service` mean here? How is the IP resolution handled?

- When should we use `imagePullPolicy: Always`? Explain the `image` pull policy shortly.

- This time, we create the `web-service.yaml` for the front-end web application `service`.
...
persistentvolumeclaim/dv-pv-claim created
service/db-service created
deployment.apps/web-deployment created
service/web-service created
```
Note that we can use `directory` with the `kubectl apply -f` command.
...
db-pv-vol   5Gi        RWO            Retain           Bound    default/dv-pv-claim   manual                  23s

$ kubectl get pvc
NAME                               STATUS   VOLUME      CAPACITY   ACCESS MODES   STORAGECLASS   AGE
dv-pv-claim   Bound    db-pv-vol   5Gi        RWO            manual         56s
```
...
- We see the home page. You can add to-dos.
...
- Create a `php-apache.yaml` file for the second application.
...
Note how the `Deployment` and `Service` objects are merged into one `yaml` file.
...
- Let's check what the web app presents us.

- On opening the browser (http://public-node-ip:node-port), we see
...
To understand better where autoscaling would provide the most value, let’s start with an example. Imagine you have a 24/7 production service with a load that is variable in time, where it is very busy during the day in the US, and relatively low at night. Ideally, we would want the number of nodes in the cluster and the number of pods in the deployment to dynamically adjust to the load to meet end-user demand. The new Cluster Autoscaling feature, together with Horizontal Pod Autoscaler, can handle this for you automatically.
...
- Add `watch` board to verify the latest status of Cluster by below Commands. (This is Optional, as not impact the Functionality of the cluster.) Observe in a separate terminal.
...
```
- php-apache Pod number increased to 2, the minimum number. 
- web-deployment Pod number increased to 3, the minimum number. 
...
- Verify the existence of `metrics-server` run by the below command

```bash
kubectl -n kube-system get pods
```

- Verify `metrics-server` can access the resources of the pods and nodes.
...
```
- Look at the values under TARGETS. The values are changed from `<unknown>/50%` to `1%/50%` & `2%/50%`, which means the HPA can now identify the current use of CPU.
...
- Open a new terminal and check the hpa.
...
- Now, let's introduce load for the to-do web app with a load-generator pod as follows (in a couple of terminals):
...
- In the terminal where we created the container with the busybox image, terminate the load generation by typing `Ctrl` + `C`. Close the load introducing terminals grafecully and observe the behaviour at the watch board.
